...
function getEventAction(array $filters)
{
    $db = dbConnect();
    $selectedEventId = $filters['id_dogadjaja'] ?? null;
    $event = getEvent($db, $selectedEventId);
    $eventTypeNames = getEventTypeNames($db);

    $events = getEventsFiltered($db, ['vrsta_dogadjaja' => $event['vrsta_dogadjaja']]);
    renderView(
        'dogadjaj',
        [
            'event' => $event,
